...
import victor.training.cleancode.kata.videostore.Statement.MoviePrice;
...
class Customer {
  ...
  public String statement() {
    List<MoviePrice> moviePrices = rentedMovies.stream()
        .map(m -> new MoviePrice(m.movie().title(), m.computePrice()))
        .toList();
...
//    return frequentRenterBonusPoints + frequentRenterPoints;
    return rentedMovies.stream().mapToInt(RentedMovie::getFrequentRenterPoints).sum();
  }
}