...
public class SplitLoop {
   ...
   public String computeStatsHard(List<Employee> employees) {
      ...
      long totalEmpAge = getTotalEmployeeAge(employees);

      retrieveSallary(consultants);
   ...
   private void retrieveSallary(List<Employee> consultants) {
      for (Employee employee : consultants) {
   ...
   private long getTotalEmployeeAge(List<Employee> employees) {
      long totalEmpAge = 0;
      for (Employee employee : employees) {
         if (!employee.isConsultant()) {
