...
public class GuardClauses {
  ...
  public int getPayAmount(Marine marine, BonusPackage bonusPackage) {
    ...
    if (bonusPackage.getValue() <= 100 && bonusPackage.getValue() >= 10) {
      throw new RuntimeException("ssdlkgsdklgsdjg");
    }
    if (isDead(marine)) {
      return DEAD_PAY_AMOUNT;
    }
    if (marine.isRetired()) { // guard clause protecting against degenerate/trivial/invalid input cases
