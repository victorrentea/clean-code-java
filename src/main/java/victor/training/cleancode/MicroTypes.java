...
public class MicroTypes {
  ...
  void primitiveObsession() {
    final Map<CustomerId, List<ProductCount>> customerOrderedProducts = fetchData();

    for (final var e : customerOrderedProducts.entrySet()) { // iterating customerOrderedProducts entries 🤢
      final String pl = e.getValue().stream()
  ...
  void lackOfAbstractions() {
    final Map<Long, List<Tuple2<String, Integer>>> map = extremeFP();
    // Joke: try "var" above :)

    for (final Long cid : map.keySet()) {
      final String pl = map.get(cid).stream()
