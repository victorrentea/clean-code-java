var DamageItemFieldset = function (damageItem, thisPageProperties) {
		...
		addChangeListeners : function () {
			...
			$("#wageCost_" + id).bind("change", function () {
				var wageCostField = new DataValidation($(this).val(), wageCostProperties),
					oldReserve = item.get("reserve");
				if (wageCostField.validate()) {
					$(this).removeClass("invalidItemField");
					item.set({wageCost : normalize($(this).val())});
				...
				if ($("#overhauling_" + id)[0].checked === true ) {
					...
					var select =  function( event, ui ) {
							...
							if (ui.item) {
								...
								$("input[id='alternativeRepairerId_"+id +"']").val(serviceProviderData.serviceProviderId).trigger('change');;
							}
							return false;
						};
						loadServiceProviders("#sonper_"+id,thisPage.damage.hdnr, thisPage.damage.filial, false,select,7);
