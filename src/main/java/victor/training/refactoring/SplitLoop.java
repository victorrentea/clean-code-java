...
public class SplitLoop {
   ...
   private String computeStats(List<Employee> employees) {
      List<Employee> nonConsultants = employees.stream().filter(e -> !e.isConsultant()).collect(toList());
      long averageAge = (long) nonConsultants.stream().mapToLong(Employee::getAge).average().orElse(0);
