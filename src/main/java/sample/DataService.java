...
public class DataService {
    ...
    private Map<Criterion, List<Precedence>> fetchCriterionPrecedenceMap(final Set<Rule> rules) {
        return rules.stream()
                .map(Rule::getFilter)
                .map(Filter::getDataSource)
                .flatMap(source -> source.getValues().stream()
                        .map(c -> Tuple.of(
                                c.getCriterion(),
                                new Precedence(source.getSourceName(), c.getValue())
                        )))
    ...
    private static class DataSourcePrecedence {
        public Set<DataFieldPrecedence> getValues() {
            return null;
        }
    ...
    private static class DataFieldPrecedence {
    ...
    private class Filter {
        public DataSourcePrecedence getDataSource() {
            return null;
        }
    }
